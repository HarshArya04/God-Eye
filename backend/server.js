...
app.get('/api/teachers/search', (req, res) => {
  const id = req.query.id?.trim().toLowerCase() || req.query.q?.trim().toLowerCase();
  if (!id) {
    return res.status(400).json({ message: 'Missing teacher id' });
  }

  const matched = teacherLocations.find(t => t.id.toLowerCase() === id);
...
app.get('/api/schedules', (req, res) => {
  ...
  if (id) {
    const matchedSchedule = schedules.find(s => s.id.toLowerCase() === id);
    if (!matchedSchedule) {
      return res.status(404).json({ message: 'Teacher schedule not found' });
    }
    return res.json(matchedSchedule);
  }
  res.json(schedules);
});
